...
(() => {
  ...
  function createRail(wrap) {
    ...
    const R = {
      ...
      drag() {
        // Pointer capture is taken only once the mouse has actually moved.
        // Capturing on pointerdown retargets the click to the rail, which
        // silently killed every link and button inside the cards.
        let down = false, dragging = false, startX = 0, startScroll = 0, pid = null;
        ...
        // stop the browser's own link/image drag from hijacking a rail drag
        rail.addEventListener("dragstart", (e) => e.preventDefault());
        const end = () => {
          if (!down) return;
          down = false;
          if (!dragging) return;                 // plain click: let it through untouched
          dragging = false;
          rail.classList.remove("is-dragging");
          // swallow only the click generated by this drag's release, then disarm
